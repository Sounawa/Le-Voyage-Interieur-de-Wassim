...
export default function Home() {
  ...
  const getMoodOverlayStyle = (): React.CSSProperties => {
    switch (currentMood) {
      case 'wonder':
        return { background: 'radial-gradient(ellipse at 50% 30%, rgba(168, 85, 247, 0.08) 0%, transparent 70%)' };
      case 'darkness':
        return { background: 'radial-gradient(ellipse at 50% 60%, rgba(20, 184, 166, 0.12) 0%, transparent 70%)' };
      case 'wisdom':
        return { background: 'radial-gradient(ellipse at 50% 40%, rgba(251, 191, 36, 0.08) 0%, transparent 70%)' };
      case 'danger':
        return { background: 'radial-gradient(ellipse at 50% 50%, rgba(236, 72, 153, 0.1) 0%, transparent 60%)' };
      case 'peace':
        return { background: 'radial-gradient(ellipse at 50% 40%, rgba(20, 184, 166, 0.08) 0%, transparent 70%)' };
      case 'triumph':
        return { background: 'radial-gradient(ellipse at 50% 30%, rgba(168, 85, 247, 0.12) 0%, transparent 60%)' };
      case 'prologue':
        return { background: 'radial-gradient(ellipse at 50% 50%, rgba(168, 85, 247, 0.06) 0%, transparent 70%)' };
      case 'ending':
        return { background: 'radial-gradient(ellipse at 50% 40%, rgba(251, 191, 36, 0.1) 0%, transparent 60%)' };
      default:
        return {};
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {!isAppReady && (
        <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-[#1a1033]">
          ...
          <p className="mt-6 text-purple-500/60 text-sm font-serif tracking-widest animate-pulse">
            Chargement...
          </p>
        </div>
      )}
      <main className={`flex-1 text-purple-100/90 transition-colors duration-1000 ${focusMode ? 'focus-mode-active' : ''}`} {...swipeHandlers.handlers}>
        {/* Background layers */}
            ...
            {!focusMode && <div className="fixed top-14 right-4 z-30 flex items-center gap-2">
              ...
              {showSettingsButton && (
                <button
                  ...
                  className="p-2.5 rounded-lg bg-[#1e1245]/80 backdrop-blur-sm border border-purple-800/15 hover:bg-purple-900/20 hover:border-purple-700/30 transition-all duration-300 group"
                  title="Paramètres"
                  aria-label="Ouvrir les paramètres"
                >
                  <Settings className="w-4 h-4 text-purple-500/50 group-hover:text-purple-400/70 transition-colors" />
                </button>
              )}
              ...
              {showSettingsButton && (
                ...
                >
                  <Search className="w-4 h-4 text-purple-500/50 group-hover:text-purple-400/70 transition-colors" />
                </button>
              )}
              ...
              {showJournalButton && (
                <button
                  ...
                  className="p-2.5 rounded-lg bg-[#1e1245]/80 backdrop-blur-sm border border-purple-800/15 hover:bg-purple-900/20 hover:border-purple-700/30 transition-all duration-300 group"
                  title="Journal de Wassim"
                  aria-label="Ouvrir le journal"
                >
                  <Scroll className="w-4 h-4 text-purple-500/50 group-hover:text-purple-400/70 transition-colors" />
                </button>
              )}
              ...
              {showGlossaryButton && (
                ...
                >
                  <BookMarked className="w-4 h-4 text-purple-500/50 group-hover:text-purple-400/70 transition-colors" />
                </button>
              )}
              ...
              {showGlossaryButton && (
                ...
                >
                  <HelpCircle className="w-4 h-4 text-purple-500/50 group-hover:text-purple-400/70 transition-colors" />
                </button>
              )}
              ...
              {showSettingsButton && (
                ...
                >
                  <MapIcon className="w-4 h-4 text-purple-500/50 group-hover:text-purple-400/70 transition-colors" />
                </button>
              )}
              ...
              {showSettingsButton && (
                ...
                >
                  <Trophy className="w-4 h-4 text-purple-500/50 group-hover:text-purple-400/70 transition-colors" />
                </button>
              )}

              {/* Reading Stats button */}
              {showSettingsButton && (
                <button onClick={() => setReadingStatsOpen(true)} className="p-2.5 rounded-lg bg-[#1e1245]/80 backdrop-blur-sm border border-purple-800/15 hover:bg-purple-900/20 hover:border-purple-700/30 transition-all duration-300 group" title="Statistiques" aria-label="Statistiques de lecture">
                  <BarChart3 className="w-4 h-4 text-purple-500/50 group-hover:text-purple-400/70 transition-colors" />
                </button>
              )}
              ...
              {showSettingsButton && (
                ...
                >
                  <GitBranch className="w-4 h-4 text-purple-500/50 group-hover:text-purple-400/70 transition-colors" />
                </button>
              )}
            </div>}
          </>
        ...
        {showReadingUI && !focusMode && (
          <button
            ...
            className="fixed top-14 left-4 z-30 p-2.5 rounded-lg bg-[#1e1245]/80 backdrop-blur-sm border border-purple-800/15 hover:bg-purple-900/20 hover:border-purple-700/30 transition-all duration-300 group"
            title="Accueil"
            aria-label="Retourner à l'accueil"
          >
            <HomeIcon className="w-4 h-4 text-purple-500/50 group-hover:text-purple-400/70 transition-colors" />
          </button>
        )}
        ...
        {showBookmarkButton && !focusMode && (
          <button
            ...
            className="fixed top-14 left-14 z-30 p-2.5 rounded-lg bg-[#1e1245]/80 backdrop-blur-sm border border-purple-800/15 hover:bg-purple-900/20 hover:border-purple-700/30 transition-all duration-300 group"
            title="Favoris"
            aria-label="Ouvrir les favoris"
          >
            <Bookmark className="w-4 h-4 text-purple-500/50 group-hover:text-purple-400/70 transition-colors" />
          </button>
        )}
      ...
      {showFooter && !focusMode && (
        <footer className="enhanced-footer footer-bottom-fade relative z-20 mt-auto py-3 px-4 text-center bg-gradient-to-t from-[#1a1033]/80 to-transparent backdrop-blur-sm border-t border-purple-900/10">
          {/* Shimmer line above footer text */}
          <div className="footer-shimmer-line" />
          <div className="flex items-center justify-center gap-2">
            <span className="icon-breathe text-purple-700/25" style={{ fontSize: '12px', '--delay': '0s' } as React.CSSProperties}>✦</span>
            <Moon className="icon-breathe footer-icon w-3 h-3 text-purple-700/30" style={{ '--delay': '0.5s' } as React.CSSProperties} />
            <span className="text-purple-700/30 text-[11px] font-serif tracking-wide">
              Le Voyage Intérieur de Wassim — Tome 1
            </span>
            <Star className="icon-breathe footer-icon w-2.5 h-2.5 text-purple-700/20" style={{ '--delay': '1.2s' } as React.CSSProperties} />
          </div>
          {/* Keyboard shortcut hint — desktop only */}
          <div className="hidden md:flex items-center justify-center gap-1.5 mt-1">
            <span className="text-purple-700/20 text-[9px] font-serif">
